...
generate_data <- function(n=1000, p=3, rho=0.97, noise=0.05,
                          scale=1.8, share=0.8, seed=2970) {
    Mu = rep(0, p)
    Sigma = matrix(rep(rho, p**2), p, p)
    #Sigma = matrix(c(1, rho, -rho, -rho, 1, -rho, -rho, rho, 1), p, p)
    diag(Sigma) <- 1

    set.seed(seed)
    X_train <- mvrnorm(n, Mu, Sigma)
    X_test <-  mvrnorm(n, Mu, Sigma)
    y_train <- sinh((share*X_train[,1] + (1-share)*X_train[,2])/scale) + rnorm(n, 0, noise)
    y_test <- sinh((share*X_test[,1] + (1-share)*X_test[,2])/scale) + rnorm(n, 0, noise)
    df_train <- data.frame(y = y_train, x1 = X_train[,1],
                           x2 = X_train[,2], x3 = X_train[,3])
    df_test <- data.frame(y = y_test, x1 = X_test[,1],
...
train_models <- function(data, seed=2970, benchmark=FALSE) {
    ...
    set.seed(seed)
    model_dt <- ctree(y~., data = df_train, control = ctree_control(maxdepth = 4))
...
# example
# interesting results for seed = 42
data <- generate_data(rho=0.9, noise=0.15, seed=42)
plot(data$train)
explainers <- train_models(data, seed = 42)
...
for (i in 1:2000) {
    print(i)
    data <- generate_data(rho=0.9, noise=0.15, seed=i)
    results[[i]] <- train_models(data, seed=i, benchmark=TRUE)
}

# obtain final result
i_best <- 377 #which.min(results) #377
data <- generate_data(rho=0.9, noise=0.15, seed=i_best)
explainers <- train_models(data, seed=i_best)
explain_models_pd(explainers)
#ggsave("figures/demo_pd.png", width=7, height=3.5)

explain_models_fi(explainers)
#ggsave("figures/demo_fi.png", width=7, height=7)
