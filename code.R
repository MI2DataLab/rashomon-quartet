...
generate_data <- function(n=1000, p=3, rho=0.97, noise=0.05, 
  ...
  y_train <- sin((share*X_train[,1] + (1-share)*X_train[,2])/scale) + rnorm(n, 0, noise)
  y_test <- sin((share*X_test[,1] + (1-share)*X_test[,2])/scale) + rnorm(n, 0, noise)
...
train_models <- function(data, seed=2970, benchmark=FALSE) {
  ...
  model_nn <- neuralnet(y~., data = df_train, hidden=c(8, 4))
...
# example
data <- generate_data(rho=0.9, noise=0.15)
...
# obtain final result 
i_best <- 1176 #which.min(results) #1176
